...
app.layout = html.Div(style={'backgroundColor': colors['background']}, children=[
    ...
    html.Div(children='Note: After multiple selections, if the map area appears to be empty, it means that there are no countries available for those combinations of selection. Please refresh to start over again.', style={
        'textAlign': 'center',
        'color': colors['text']
    }),
    html.Br(),
...
def create_time_series(df, axis_type, title):
    ...
    fig = px.bar(
        ...
        title=f'Artist vs Stream Count for "{c_val}"',  # figure title
...
def create_track_acoustic(df, axis):
    ...
    #print(t_val)
    testarr=testarr = [random.uniform(0, 0.7), random.uniform(0, 0.9), random.uniform(0, 0.994),
